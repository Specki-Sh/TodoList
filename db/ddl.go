...
const (
	...
	SelectByIDUser               = `SELECT id, name, email, password_hash FROM users WHERE id = $1`
	SelectAllUsers               = `SELECT u.id, u.name, u.email, u.password_hash, t.id, t.title, t.description, t.due_date, t.priority, t.completed FROM users u LEFT JOIN tasks t ON u.id = t.user_id`
	SelectByEmailAndPasswordUser = `SELECT id, name, email, password_hash FROM users WHERE name = $1 AND email = $2`
)
...
const (
	...
	UpdateByIDUser = `UPDATE users SET name=$1, email=$2, password_hash=$3 WHERE id=$4`
)
